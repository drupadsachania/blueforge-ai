...
**Source:** https://docs.cloud.google.com/chronicle/docs/detection/yara-l-2-0-overview/  
**Scraped:** 2026-03-05T09:31:12.676518Z
...
Google secops
SIEM
The following examples show queries written in YARA-L 2.0. Each example demonstrates how to correlate events within the query rule language.
Rules and tuning
The following rule checks for specific patterns in event data and creates a detection
if it finds the patterns. This rule includes a variable
$e1
for tracking event
type and
metadata.event_type
UDM field. The rule checks for specific occurrences
of regular expression matches with
e1
. When the event
$e1
takes place, a detection is created.
A
not
condition is included in the rule to exclude certain non-malicious paths.
You can add
not
...
USER_LOGIN
.
For that event group, the rule calls the user ID as
$user
and the login city as
$city
.
...
values (denoted by
#city
) is greater than 1 in the event group (
$udm
...
corresponds to
USER_CREATION
events and calls the user ID as
$user
.
$user
is used to join the two groups of events together.
$delete
corresponds to
USER_DELETION
events and calls the user ID as
$user
. This rule looks for a match where the user identifier in the two event groups is the same.
This rule looks for cases where the event from
$delete
happens later than the event from
$create
...
section and a
condition
section only checking for the existence of 1 event (for example, "$e", "#e > 0", "#e >= 1", "1 <= #e", "0 < #e").
...
This example shows a single-event rule that uses the
match
section to find any user with at least one login event occurring within a 5-minute time window.It checks for the existence of a user login event.
rule SingleEventRule {
  meta:
...
A
match
section which specifies the time range over which events need to be grouped.
A
condition
section specifying what condition should trigger the detection and checking for the existence of multiple events.
...
Single event within a range of IP addresses
The following example shows a single event rule that searches for matches
between two specific hostnames and a specific range of IP addresses:
...
The following rule searches for login events where all source IP addresses don't match an IP address known to be secure within a timespan of 5 minutes.
rule SuspiciousIPLogins {
  ...
  events:
    ...
    // Detects if all source IP addresses in an event don't match "100.97.16.0"
    // For example, if an event has source IP addresses
    // ["100.97.16.1", "100.97.16.2", "100.97.16.3"],
    // it will be detected since "100.97.16.1", "100.97.16.2",
    // and "100.97.16.3" all don't match "100.97.16.0".
...
The following YARA-L 2.0 regular expression example searches for events with emails received from the altostrat.com domain. Since
nocase
...
rule composite_tactic_aggregation {
  ...
  outcome:
    ...
    $raw_score = 50 + (15 * $mitre_tactics_count)
    $risk_score = if($raw_score > 100, 100, $raw_score)
    $upstream_rules = array_distinct($rule_name)
...
.
However, for other referenced event fields, zero values aren't excluded unless you explicitly specify such conditions.
...
conditionals on outcome variables. You can use the
outcome
...
. If the value of
$hostname
matches over a 5-minute period,
then a severity score is applied. When including a time period in the
match
section,
...
Refactor a multi-event outcome rule into a single-event outcome rule
You can use the
outcome
section for both single-event rules (rules without a
match
section), and multi-event rules (rules with a
match
section).
...
You can refactor the rule by deleting the
match
section. Note that you
must also remove the aggregate in the
outcome
section since the rule will now be
single-event. For more information on aggregations, see
...
In the
condition
section, you can use outcome variables that were defined
in the
outcome
