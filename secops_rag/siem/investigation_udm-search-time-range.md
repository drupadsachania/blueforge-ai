...
**Scraped:** 2026-03-05T09:32:26.693426Z
...
From the
Range
tab (the default tab), adjust the time range by selecting 
...
and
End
fields to choose a more specific
...
Search History
is already enabled for your account.
Click
more_vert
and select
Opt into search history
.
...
Run multiple UDM searches
: While a search query is in progress, you can
...
View query results
: Scroll through the query history and select search
results within 24 hours of running a query. Click
History
and select one
...
cached
Rerun
to run the query against the latest data. This new run is 
added to the search history and the results are made available when the 
query completes.
Share query results
: Copy the URL of the query results to share them 
with other users.
...
search query is still available in the
History
pane. You can rerun your
