...
**Scraped:** 2026-03-05T09:32:39.412139Z
...
Click the Gemini logo to open the Gemini pane.
Enter a natural language prompt and press
Enter
...
If the generated search query meets your requirements, click
Run search
.
...
For better results, we recommend using the
Gemini pane to generate
search queries
.
...
Figure 2.
Enter a natural language search and click
Generate Query
.
...
message:
"Sorry, no valid query could be generated. Try asking a
different way."
Review the generated search query. The syntax is
Overview of YARA-L
2.0
.
...
Provide feedback about the query using the
Generated Query
feedback
icons. Select one of the following:
If the query returns the expected results, click
thumb_up
Thumbs Up
.
If the query does not return the expected results, click
thumb_down
Thumbs Down
.
Optional: Include additional detail in the
Feedback
field.
