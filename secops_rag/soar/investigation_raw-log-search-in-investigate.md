...
**Scraped:** 2026-03-05T10:05:22.876015Z
...
Google secops
SIEM
This document explains how you can use Google Security Operations to search the raw logs ingested into your
Google SecOps tenant and get relevant context, including associated events and
entities.
Raw log searches correlate raw events with their generated UDM events. A raw log search helps you to identify
normalization gaps and identify unparsed logs that aren't being processed by the parsers.
...
raw =
to your search and
enclose your search term in quotation marks (for example,
raw =
"example.com"
).
...
specific sources, as opposed to all of your log sources. From the
Log
sources
menu, select one or more log sources (the default is
all
).
Regular expressions: Use a regular expression. For example,
raw =
/goo\w{3}.com/
would match against
google.com
...
arrow_drop_down
Arrow drop down
to collapse or expand the graph.
Raw log results
...
To download raw log results to a CSV file, on the
Raw log
results table,
click
more_vert
...
, and
Raw Log
columns is saved. You can use the
Column manager
to select which columns to
download. The
Raw Log
