...
**Scraped:** 2026-03-05T10:06:16.319059Z
...
Use the
$
character, followed by a variable name, to define an outcome variable in the
outcome
section of a single query. You can define up to 20 outcome variables. The variable names themselves are arbitrary. For rules, outcome values are computed and aggregated based on each detection.
...
The
outcome
section performs aggregations on the event and placeholder variables: count the failed logins, count the distinct IPs, and get the time the failure occurred.
outcome:
   $failed_login_count = count($e.metadata.id)
   $first_fail_time = min($e.metadata.event_timestamp.seconds)
If you include and populate the special
$risk_score
outcome variable, its value (integer or float) is displayed on the
Alerts and IoCs
page for alerts generated by the query.
...
$risk_score
is set to 40.
If the query is not configured to generate an alert, then
$risk_score
is set to 15.
The value of
$risk_score
...
Google SecOps Risk Analytics automatically associates detections
and alerts back to entities related to that detection or alert. The
risk_score
outcome variable is used to assign an amount of risk. If this value is not set,
the default detection or alert value is used. You can configure default values
in Settings.
To ensure consistency across the platform, we recommend using the following
score ranges when assigning a
risk_score
to your custom detections. This
alignment helps standardize alert prioritization and response workflows.
Severity
Score range
Description
Example
Alerting - Critical
90 - 100
Active compromise with the potential to have impact beyond a single user account or endpoint. Requires immediate review.
Mimikatz executed on Domain Controller.
Alerting - High
80 - 89
Active compromise of a single endpoint or entity.  Should receive immediate review.
Production server calling out to recent, known C2.
Alerting - Medium
50 - 79
Potential security issue requiring investigation. No confirmed compromise but escalation is possible.
An exposed credential, no signs of misuse.
Non-Alerting - Low
20 - 49
Low-impact security event that, when combined with other indicators or observations, could lead to a more significant incident. No review generally needed, can be combined with other detections through composite rules to create an alert.
Internal port scan.
Non-Alerting Observations
1 - 19
Generally, information based detections intended to build situational awareness of a threat. Generally does not require review; can be combined with other detections through composite rules to generate alerts.
A login event, no signs of misuse.
Because
risk_score
is an outcome variable, your rules can express nuance
depending on factors such as threat intelligence or other concurrent conditions.
Entity risk scores can be used to generate risk-based alerts in near real time.
...
In addition to the
risk_score
outcome variable, you can also specify the
risk_entity_to_score
outcome variable. This variable lets you explicitly
designate which entity or entities from a rule detection should apply that
detection's risk score.
You can create multiple
risk_entity_to_score
variables by appending a unique
string to the variable and assign the risk score to specific entities. For
example:
$risk_entity_to_score_source_asset = $e.principal.asset.hostname
$risk_entity_to_score_targeted_user = $e.target.user.userid
These outcome variables assign the
risk_score
...
This rule detects successful logins to high-privilege administrator accounts. It
uses the
risk_entity_to_score
keyword to explicitly target the specific user
being logged in. Using this variable, the detection ensures that the calculated
risk score applies to only to that user and not to other entities involved in
the event, such as the source IP address.
For details about how to write a multi-event rule, see
Multiple event rule
.
...
This rule checks for lateral movement by identifying network connections between
internal network segments. It uses
$risk_entity_to_score prefix
to assign risk
to multiple entities within a single detection. Both the initiating host
(
$risk_entity_to_score_source_asset
) and the targeted user
(
$risk_entity_to_score_targeted_user
) have their risk scores updated. The risk
engine records other outcome variables that don't use this specific prefix for
context, but ignores them (their risk scores remain the same).
For details about how to write a multi-event rule, see
Multiple event rule
and
...
You can read a conditional expression as "if BOOL_CLAUSE is true, then return
THEN_CLAUSE, else return ELSE_CLAUSE".
BOOL_CLAUSE must evaluate to a boolean value. A BOOL_CLAUSE expression takes a
similar form as expressions in the
events
...
You can omit the ELSE_CLAUSE if the data type is integer or a float. If omitted, the
ELSE_CLAUSE evaluates to 0. For example:
`if($e.field = "a", 5)` is equivalent to `if($e.field = "a", 5, 0)`
You must provide the ELSE_CLAUSE if the data type is string or if the THEN_CLAUSE
is a placeholder variable or outcome variable.
Mathematical operations
You can use mathematical operations to compute integer or float data type in the
outcome
and
events
...
The following snippet is an example computation in the
outcome
section:
outcome:
  $risk_score = max(100 + if($severity = "HIGH", 10, 5) - if($severity = "LOW", 20, 0))
Mathematical operations are allowed on the following types of operands as long as
each operand and the entire arithmetic expression is properly aggregated (See
Aggregations
):
...
events
section
Numeric outcome variables defined in the
outcome
section
...
placeholder variables defined in the
events
...
not be aggregated (See
Aggregations
).
In the following example, the outcome variable
$risk_score
derives its
value from the outcome variable
$event_count
:
...
multiple values. For example, the event field variable
$e.target.ip
is a repeated field
...
Similarly, both non-repeated event fields and repeated event fields used in the
outcome
section
...
For example, if a 5-minute window contains 5
$e
events,
$e.principal.hostname
in the outcome section points to five different hostnames. The event field variable
$e.principal.hostname
is treated as a non-scalar value in the
outcome
...
match
section
Event placeholders (repeated or non-repeated) when the query uses a
match
section
Repeated event fields when the query doesn't use a
match
section
Repeated event placeholders when the query doesn't use a
match
section
Scalar event fields, scalar event placeholders, and constants can be wrapped in
aggregation functions in queries that don't include a
match
section. However, in
...
The aggregate function is important when a rule includes a
condition
section
...
The
outcome
section can't reference a new placeholder variable which wasn't already defined in the
events
section or in the
outcome
section.
The
outcome
section can't use event variables that have not been defined in the
events
section.
The
outcome
section can use an event field that was not used in the
events
section, given that the event variable that the event field belongs to was already defined in the
events
section.
The
outcome
section can only correlate event variables that have already been correlated in the
events
...
for details on detection deduping with the
outcome
section.
What's next
